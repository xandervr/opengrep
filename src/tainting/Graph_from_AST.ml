 ...
 *)
type fn_id = IL.name option list [@@deriving show, eq, ord]
  ...
  | path ->
      ...
      |> List.map (fun name_opt ->
             Option.value ~default:"<anon>"
               (Option.map (fun name -> fst name.IL.ident) name_opt))
      |> String.concat "::"
...
let equal_with_pos f1 f2 =
  let equal_il_name n1 n2 =
    Function_id.equal (Function_id.of_il_name n1) (Function_id.of_il_name n2)
  in
...
let remove_known_source_extension (segment : string) : string =
  ...
  | Some idx when idx > 0 ->
      ...
      in
      if is_known_source_extension ext then String.sub segment 0 idx
      else segment
  | _ -> segment
...
let normalize_bash_command_call_for_graph ~(lang : Lang.t) (callee : G.expr)
  ...
  | Lang.Bash, G.N (G.Id (("!sh_cmd!", _), _)), first_arg :: rest_args -> (
      match bash_command_name_of_arg first_arg with
      | Some (cmd_name, cmd_tok) ->
          let cmd_callee =
            {
              callee with
              G.e = G.N (G.Id ((cmd_name, cmd_tok), G.empty_id_info ()));
            }
          in
...
let token_of_resolved_call ~(lang : Lang.t) ~(original_call : G.expr)
  ...
  | G.DotAccess (_, _, G.FN (G.Id (("new", _), _))) when Lang.(lang =*= Ruby)
    -> (
  ...
  | _ -> (
      ...
      | Some arity -> (
          ...
          | [] ->
              Log.debug (fun m ->
                  m
                    "PICK_BY_ARITY: %d candidates, none with arity %d; giving \
                     up"
          ...
          | _ ->
              ...
              Log.debug (fun m ->
                  m
                    "PICK_BY_ARITY: %d candidates, %d still match arity %d; \
                     giving up"
                    (List.length matches)
                    (List.length arity_matches)
                    arity);
              None)
      | None ->
          Log.debug (fun m ->
...
let pick_imported_match (id_info : G.id_info) call_tok (call_arity : int option)
  ...
  | Some (G.ImportedEntity canonical, _) ->
      ...
      let matches =
        ...
        | None -> []
      in
      let result = pick_with_caller_relative_module module_canonical matches in
      Option.iter
        (fun fn_id ->
          Log.debug (fun m ->
              m "CALL_EXTRACT: Resolved imported call %s to %s"
                (String.concat "." canonical)
                (show_fn_id fn_id)))
        result;
      result
  | Some (G.ImportedModule canonical, _) ->
      ...
      in
      let result = pick_with_caller_relative_module canonical matches in
      Option.iter
        (fun fn_id ->
          Log.debug (fun m ->
...
let fn_id_of_entity ~(lang : Lang.t) (opt_ent : G.entity option)
    (parent_path : IL.name option list) (fdef : G.function_definition) :
    fn_id option =
        ...
        | G.N (G.Id ((_, tok), id_info)) -> (
            ...
            | None -> (
                ...
                | Some fn_id ->
                    Log.debug (fun m ->
                        m
                          "CALL_EXTRACT: Found unresolved Id that is a \
                           function, adding as implicit call");
...
let extract_toplevel_calls ~(lang : Lang.t) ?(object_mappings = [])
  ...
  Log.debug (fun m ->
      m "CALL_EXTRACT: Starting extraction for top-level statements");
  let calls = ref [] in
  ...
  List.iter
    (fun func ->
      ...
      | Some (loc_start, loc_end) ->
          ...
          func_ranges :=
            (loc_start.pos.file, range.start, range.end_) :: !func_ranges
      | None -> ())
    all_funcs;
...
let extract_callback_from_arg (arg_expr : G.expr) :
    (IL.name * Tok.t * IL.name option) option =
  ...
  let nested_match =
    List.find_opt
      (fun f ->
        match List_.init_and_last_opt f.fn_id with
        | Some (f_parent, Some name)
          when String.equal (fst name.IL.ident) callback_name_str ->
  ...
  | None -> (
      ...
      let class_method_match =
        ...
        | Some cls ->
            ...
            pick_method_in_lineage class_hierarchy all_funcs None class_name_str
              callback_name_str
        | None -> None
      in
      ...
      | Some fn_id ->
          Log.debug (fun m ->
              m "HOF_EXTRACT: Found class method callback %s" callback_name_str);
          Some fn_id
      | None -> (
          ...
          let top_level_match =
            List.find_opt
              (fun f ->
                match f.fn_id with
                | [ None; Some name ] when fst name.IL.ident = callback_name_str
                  ->
                    true
                | _ -> false)
              all_funcs
          ...
          | Some f ->
              Log.debug (fun m ->
                  m "HOF_EXTRACT: Found top-level callback %s" callback_name_str);
              Some f.fn_id
          | None ->
              Log.debug (fun m ->
...
let extract_hof_callbacks_from_call ~method_hofs ~function_hofs ~class_hierarchy
    ~all_funcs ~caller_parent_path (callee : G.expr) (args : G.arguments) :
  ...
  let all_callback_args = Tok.unbracket args |> List.filter_map try_arg in
...
let extract_hof_callbacks ?(_object_mappings = []) ?(class_hierarchy = [])
    ?(all_funcs = []) ?(caller_parent_path = []) ~(lang : Lang.t)
    (fdef : G.function_definition) : (fn_id * Tok.t * IL.name option) list =
      ...
      method! visit_expr env e =
        ...
        | G.Call
          ...
          when Lang.(lang =*= Ruby || lang =*= Scala) ->
            ...
            in
            let found =
              extract_hof_callbacks_from_call ~method_hofs ~function_hofs
                ~class_hierarchy ~all_funcs ~caller_parent_path callee
                merged_args
            in
        ...
        | G.Call (callee, args) ->
            let found =
              extract_hof_callbacks_from_call ~method_hofs ~function_hofs
                ~class_hierarchy ~all_funcs ~caller_parent_path callee args
            in
            callbacks := found @ !callbacks
        | _ -> ());
...
let build_call_graph ~(lang : Lang.t) ?(object_mappings = []) (ast : G.program)
      ...
      | Some fn_id ->
          ...
          List.iter
            (fun (callee_fn_id, call_tok) ->
              ...
              | Some callee_node, Some caller_node ->
                  Call_graph.add_edge graph ~src:callee_node ~dst:caller_node
                    ~call_tok;
                  if is_toplevel_lambda then
                    Call_graph.add_edge graph ~src:callee_node
                      ~dst:top_level_node ~call_tok
              | _ -> ())
            callee_calls;
          ...
          let callback_calls =
            extract_hof_callbacks ~_object_mappings:object_mappings
              ~class_hierarchy ~all_funcs:funcs ~caller_parent_path:fn_id ~lang
              fdef
          in
              ...
              | Some callback_node, Some caller_node ->
                  let src_to_caller =
                    ...
                    | Some tmp_name ->
                        ...
                        Call_graph.add_edge graph ~src:callback_node
                          ~dst:tmp_node ~call_tok;
                        tmp_node
                    | None -> callback_node
                  in
                  Call_graph.add_edge graph ~src:src_to_caller ~dst:caller_node
                    ~call_tok;
                  if is_toplevel_lambda then
                    Call_graph.add_edge graph ~src:src_to_caller
                      ~dst:top_level_node ~call_tok
              | _ -> ())
            callback_calls
      | None -> ())
  ...
  List.iter
    (fun (callee_fn_id, call_tok) ->
      ...
      | Some callee_node ->
          Call_graph.add_edge graph ~src:callee_node ~dst:top_level_node
            ~call_tok
      | None -> ())
    toplevel_calls;
  Log.debug (fun m ->
      m "CALL_GRAPH: Added %d edges from top-level calls"
        (List.length toplevel_calls));
  ...
  let toplevel_hof_callbacks =
    ...
    let function_hofs =
      ...
      |> List.filter_map (function
           ...
           | _ -> None)
    in
    Visit_function_defs.fold_toplevel_calls
      (fun acc _call_e callee args ->
        let found =
          extract_hof_callbacks_from_call ~method_hofs ~function_hofs
            ~class_hierarchy ~all_funcs:funcs ~caller_parent_path:[] callee args
        in
        found @ acc)
      [] ast
  in
  toplevel_hof_callbacks
  |> List.iter (fun (callback_fn_id, call_tok, tmp_opt) ->
         ...
         | Some callback_node ->
             let src_to_caller =
               ...
               | Some tmp_name ->
                   let tmp_node = Function_id.of_il_name tmp_name in
                   Call_graph.add_edge graph ~src:callback_node ~dst:tmp_node
                     ~call_tok;
                   tmp_node
               | None -> callback_node
             in
             Call_graph.add_edge graph ~src:src_to_caller ~dst:top_level_node
               ~call_tok
         | None -> ());
  Log.debug (fun m ->
      m "CALL_GRAPH: Added %d edges from top-level HOF callbacks"
        (List.length toplevel_hof_callbacks));
    ...
    (fun func ->
      ...
      if Object_initialization.is_constructor lang func_name class_name_str then
        ...
        List.iter
          (fun method_func ->
            match
              (fn_id_to_node func.fn_id, fn_id_to_node method_func.fn_id)
            with
            | Some constructor_node, Some method_node ->
                if
                  not (Call_graph.G.mem_edge graph constructor_node method_node)
                then
                  Call_graph.add_edge graph ~src:constructor_node
                    ~dst:method_node
    ...
    object (self)
      ...
      method! visit_definition (env : unit) ((ent, def_kind) as def) =
        ...
        | G.ClassDef cdef ->
            ...
            let cbody_range_opt =
              AST_generic_helpers.range_of_any_opt (G.Flds cbody_stmts)
            in
            ...
            | Some (loc_start, loc_end) ->
                ...
                List.iter
                  (fun ((range, match_file) : Range.t * Fpath.t) ->
                    if
                      Fpath.equal class_file match_file
                      && class_start <= range.Range.start
                      && range.Range.end_ <= class_end
                    then
                      (* This class contains this range - add it to the list *)
                      match !current_class with
                      | Some class_g_name ->
                          let class_il_name =
                            AST_to_IL.var_of_name class_g_name
                          in
                          let class_str = fst class_il_name.IL.ident in
                          ...
                          if
                            not
                              (List.exists
                                 (fun (fid, _) -> equal_fn_id fid class_fn_id)
                                 existing)
                          then
                            Hashtbl.replace range_to_funcs key
                              ((class_fn_id, class_size) :: existing)
                      | None -> ())
                  ranges;
        ...
        | G.VarDef { vinit = Some { e = G.Lambda fdef; _ }; _ } -> (
            ...
            let func_range_opt =
              AST_generic_helpers.range_of_any_opt (G.Def def)
            in
                ...
                List.iter
                  (fun ((range, match_file) : Range.t * Fpath.t) ->
                    ...
                    then
                      ...
                      in
                      match
                        fn_id_of_entity ~lang (Some ent) visitor_parent_path
                          fdef
                      with
                      | Some fn_id ->
                          let key = (range, match_file) in
  ...
  List.fold_left
    (fun matching_funcs range ->
      ...
      if List.is_empty funcs_list then
        ...
        let sorted =
          List.sort
            (fun (_, size1) (_, size2) -> compare size1 size2)
            funcs_list
        in

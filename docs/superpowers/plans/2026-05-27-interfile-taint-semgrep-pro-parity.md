...
- Pre-existing interfile findings gap (NOT caused by the timeout fix): on this local macOS build, five e2e fixtures lose findings — `taint_interfile_js_imported_value` (0/2), `taint_interfile_js_object_method` (0/1), `taint_interfile_js_propagator` (0/1), `taint_interfile_js_sanitizer` (2/1), and `taint_interfile_js_commonjs` (2/3). Verified identical on the committed HEAD binary AND with codex's source/sanitizer/propagator formula prefilter disabled, so it is neither the timeout fix nor the prefilter. It originates earlier (the `c32d3dde2` precompute refactor) or is a local-macOS-vs-Docker build difference, since codex verified those fixtures green via Docker. Needs separate investigation against a Docker build before any parity claim.

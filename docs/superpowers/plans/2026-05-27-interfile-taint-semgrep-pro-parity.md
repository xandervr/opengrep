...
- Interfile precompute timeout fix (full default ruleset): the prior checkpoints were measured with a single-rule `test.yaml`. Running the user's real command on the full 1059-rule default set (`opengrep scan ./trafficlists-app/ --taint-interfile --dataflow-traces`) produced an endless stream of `Timeout on <file>` warnings, one roughly every 29s, and never finished. Root cause: with the full ruleset ~18 heavy JS security rules exceed the per-rule 5s timeout during the shared interfile precompute. Once 3 rules timed out, the per-file `timeout_threshold` raised `File_timeout`, which escaped `check_taint_signatures` *before* the result was cached in `interfile_result_cache` — so the precompute (held under the cache mutex) was re-run and re-failed for every interfile target, serializing all worker domains. Fix: `Match_rules.check_taint_signatures` now disables the per-file threshold for the precompute (the precompute target is the synthetic whole-repo AST, not a single skippable file) while keeping the per-rule 5s cap, so a slow rule degrades into a recorded per-rule timeout error and the function returns/caches normally; each `run` also gets a fresh per-rule timeout counter. `Core_scan.mk_target_handler` additionally wraps the precompute in a `try` that caches a degraded empty cache on `File_timeout`/`Out_of_memory`/`Stack_overflow`/`ExceededMemoryLimit`, guaranteeing the shared precompute is attempted at most once per `(analyzer, rule-set)` key. After the fix the full-ruleset ANPR command completes in ~312s (text mode) with **0 `Timeout on` warnings** and a clean 5-finding report; `--json` shows `results=5`, `interfile=["Bash","JavaScript"]`, with 18 remaining per-rule `Timeout` errors all attributed to one heavy file (`notificationService.js`) and not surfaced in text output. The single-rule JSON repro is unchanged (`results=0`, `errors=0`, `interfile=["JavaScript"]`), and the e2e taint-interfile direct-scan matrix is unchanged (correctness-neutral).
- Pre-existing interfile findings gap — ROOT-CAUSED and FIXED. Five e2e fixtures lost findings on the committed HEAD: `taint_interfile_js_imported_value` (0/2), `taint_interfile_js_object_method` (0/1), `taint_interfile_js_propagator` (0/1), `taint_interfile_js_sanitizer` (2/1), `taint_interfile_js_commonjs` (2/3). Bisection proved they all PASS at `5ef3df181` (the parent of the `c32d3dde2` "speed up interfile taint precompute" commit) and FAIL at HEAD, so `c32d3dde2` regressed them (it was verified for performance, `results=0` on ANPR, but the correctness matrix was not re-run). Root cause: `c32d3dde2` split interfile taint into (1) a per-function *signature* precompute over the merged whole-repo AST (`signatures_only`) and (2) a per-file re-analysis that runs on a single file's AST plus the cached signatures. The per-file phase only sees one file, so any cross-file flow not expressible as a per-function signature is lost: module-level imported values (no function carries them), object-literal methods (call-graph keying), and — because the precompute's `signature_graph` was the forward-reachable-from-sinks subgraph, which excludes the *callees* of sink functions — imported propagator/sanitizer functions whose effects are never summarized. The parent worked because it analyzed the merged AST directly per target (correct but O(targets) and unusably slow: the single-rule ANPR repro timed out at >400s).
- Fix (correct AND performant): the interfile analysis now runs the bounded merged-AST taint analysis ONCE per `(analyzer, rule-set)`, records its findings, caches them, and distributes them to each file by match location (`keep_matches_in_file`); the buggy per-file re-analysis is dropped. Concretely, in `Match_tainting_mode.check_rule`: (a) the function-pass and module-level `record_matches` calls now run in the interfile precompute (gated on `not use_precomputed_signatures` instead of `not signatures_only`), so the precompute -- which already sees the full merged AST, the cross-file call graph, and the merged global env -- emits concrete findings; (b) the precompute's `signature_graph` now uses the same relevant subgraph as the regular analysis (which includes sink-function callees), so imported propagator/sanitizer signatures are summarized; (c) the top-level and object-initialization passes run in the precompute too but bounded to the source/sink files (`toplevel_scope_ast`) and the top-level pass is skipped entirely when no source/sink sits at module scope (`has_top_level_specs`), so they do not become O(repo). `Core_scan.mk_target_handler` now caches `Core_result.matches_single_file` per rule-set and returns `keep_matches_in_file` of it; the per-file `Match_rules.check ~interfile_taint_cache` call is removed. Result: ALL 87 parsed e2e interfile test cases pass with exact expected counts and 0 errors (the 5 regressed fixtures + the previously-passing language matrix, parser smoke, CommonJS, inheritance, provider, etc.), intrafile taint and search modes are unchanged, and the full-ruleset ANPR command completes in ~319s with **0 `Timeout on` warnings** and the same 5 findings / same 18 per-rule heavy-rule `Timeout` errors as the prior baseline (i.e. no new timeouts, no findings lost). One test had an internally inconsistent assertion (`len == 3` alongside a 4-element expected-paths list for the `constructor_tuple` case added in `9c24e76bb`); corrected to `== 4` to match its own path/line lists.
- Known remaining edge: the heavy custom `test.yaml` SSRF rule (broad `req.*`/`process.env` sources, many `http`/`fetch` sinks) now reaches the default per-rule 5s cap during the merged precompute on the 1072-file ANPR repo, so the single-rule JSON repro reports `errors=1` (one recorded `Timeout`) while still `results=0` (there is no real SSRF flow in ANPR). It is the broader relevant-subgraph that pushes that one rule over 5s; the default-ruleset rules are unaffected (identical 18 timeout rules before/after). Raising `--timeout` clears it. A tighter signature_graph (callees-of-sinks only, instead of the full relevant subgraph) would remove this edge but is left for a future, separately-verified change.
- Multi-hop cross-file taint — ROOT-CAUSED and FIXED. An external proof harness (`_opengrep-crossfile-proof/`, a 1-rule `os.Args` -> `exec.Command` Go taint rule, no `options.interfile`, driven by `--taint-interfile`) showed single-hop cross-file taint worked but a 2-hop chain `A -> M -> B` was dropped when the second hop crossed a file boundary: case 5 (`A` in f1, `M`+`B` in f2) found it, but case 4 (`A`+`M` in f1, `B` in f2) and case 3 (all three split) found nothing. Root cause: `Taint_signature_extractor.extract_signature` dropped any `ToSink` effect whose sink range was located in a different file than the function being summarized (`sink_is_in_function_file`, added in the original `7fcd695b5` interfile feature). A forwarder `M` that passes a tainted parameter into a callee `B` whose `exec.Command` sink lives in another file therefore never recorded a `ToSink` in its own signature, so `M`'s callers could not see that `M` reaches a sink — transitive cross-file composition was severed at every file boundary. (Intra-file forwarding and single cross-file hops worked because the sink was either in the same file or applied directly during the recording pass, not during signature extraction.) Fix: removed the `sink_is_in_function_file` filter (and its now-dead `function_file`/`normalize_file` helpers). A `ToSink` in a function's signature truthfully means "a tainted parameter/state reaches a sink", independent of which file the sink physically sits in; the finding is still reported at the real sink location. In single-file/intra-file analysis every sink is already in the function's file, so the change is a no-op there. Verified: proof cases 1-5 all find the flow, a 4-file/4-hop chain (`A->M1->M2->B`) reports at the sink, `known-bug-multihop/` reports `c_sink.go:9`, and the proof's escalating modes are exactly default=1 / `--taint-intrafile`=2 / `--taint-interfile`=3. All 87 e2e interfile test cases still pass with exact counts and 0 errors; intra-file taint and search modes unchanged. ANPR full-ruleset findings are unchanged (same 5), with the cost of composing more cross-file summaries showing as ~366s (vs ~319s) and one additional borderline rule (`raw-html-format`) reaching the 5s cap (19 vs 18 timeouts, no finding lost) — an acceptable trade for correctly tracking multi-hop chains such as the real `connector.RunCommand -> connection.RunCommand -> runWithContext -> session.Run` SSH path.
- Interfile per-rule timeout floor (15s). Because each inter-file rule is analyzed once over the merged whole-repo AST, the regular 5s per-rule timeout cut heavy rules short on large repos. `Core_scan` now floors the inter-file precompute's per-rule timeout at `interfile_min_rule_timeout = 15.0s` (`raise_timeout_for_interfile`), still honoring a larger explicit `--timeout` and `--timeout 0` (no limit); regular per-file (intra-file) rules keep the base `--timeout`. Value chosen with the user as a moderate balance: measured on ANPR, the heavy custom SSRF rule needs ~30s to fully complete and the full-ruleset scan at a 30s floor exceeded ~14 min, so 30s was rejected as a default; 15s lets rules needing 5-15s complete while bounding the worst case. The floor is documented in the `--taint-interfile` and `--timeout` help text. Verified: all 87 e2e cases still pass with exact counts; with the floor the single-rule ANPR repro runs ~40s (was ~18s at the 5s cap) and `--timeout 60` lets the heavy rule complete (`errors=0`).
...
**Immediate resume point:** the full-default-ruleset timeout storm is fixed (see the precompute timeout checkpoint above). Two follow-ups remain. (1) Reduce the ~312s ANPR full-ruleset cost: the biggest remaining lever is the precompute's full-pass + bounded `limit_to_taint_files` fallback running both passes when any rule times out (`check_taint_signatures` runs `run ()` then re-runs `run ~limit_to_taint_files:true`); this is correctness-sensitive because it decides which signatures exist, so profile before changing it. (2) Investigate the pre-existing interfile findings gap (five JS fixtures, see checkpoint above) against a Docker build to confirm whether it is a real engine regression or a local-build artifact. Do not reopen generic/regex unless the user explicitly wants non-Semgrep-Pro behavior for those extended analyzers.
...
3. Keep BOTH performance repros in the loop when touching interfile precompute, run each at least twice to filter local noise, and compare JSON result/error counts:
   - Single-rule (fast signal): `bin/opengrep scan -f /Users/xander/Documents/Work/Focus/Projects.nosync/ANPR/test.yaml /Users/xander/Documents/Work/Focus/Projects.nosync/ANPR/trafficlists-app --taint-interfile --dataflow-traces --json`.
   - Full default ruleset (the user's real command; this is where the timeout storm appeared): `opengrep scan ./trafficlists-app/ --taint-interfile --dataflow-traces` from the ANPR dir. Confirm 0 `Timeout on` warnings and that the scan completes.
